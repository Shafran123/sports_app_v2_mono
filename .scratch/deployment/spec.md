...
| Q5/Q8 | Image persistence | Railway **volume** mounted at `sp_be/uploads` + a `/uploads/:path*` rewrite on both Vercel apps (mirrors the existing `/api` rewrite). Zero backend code change for uploads; legacy local files seeded into the volume once. Supabase Storage (ADR-0010) remains a follow-up. |
...
github.com/Shafran123/sports_app_v2_mono (main)
  ...
  └─ Railway: sp_be           → sports-be.up.railway.app      (env: DATABASE_URL, FRONTEND_URL, SOCKET_ALLOWED_ORIGINS, …)
       └─ volume: /app/sp_be/uploads   (persistent images)
          └─ Supabase Postgres (fresh) ← DATABASE_URL
```

Browser traffic: REST flows same-origin through each app's Vercel rewrites → Railway `/api/v1/*`. Socket.io flows **directly** browser → Railway (hence `SOCKET_ALLOWED_ORIGINS`). Images flow `/uploads/*` → Vercel rewrite → Railway volume. PayHere notify is hit by PayHere's servers → `API_PUBLIC_URL` (Railway URL, must be reachable from the internet).
...
3. Railway: import repo (root dir `sp_be`), nixpacks build, env vars, **volume** mounted at `/app/sp_be/uploads`, deploy; healthcheck `GET /health` 200; `npm run db:setup` once; copy local `sp_be/uploads/*` into the volume.
...
- [ ] User app: sign-in (Firebase), browse venues, venue detail renders `/uploads` images through the rewrite.
...
- [ ] Photo upload from admin persistence: upload → trigger Railway redeploy → image still 200 after redeploy (volume).
- [ ] Jobs: `startReminderJob` runs on boot; daily digest fires 06:06 Asia/Colombo.

## Rollback

- Railway: previous deployment; Vercel: redeploy previous commit. Uploads live on the volume, unaffected by app rollbacks.
- DB: pre-prod only — point `DATABASE_URL` at the old pre-prod DB if needed.

## Known Issues / Follow-ups (deferred by design)

- Photos live on the Railway volume, not object storage — ephemeral if the volume is ever destroyed (ADR-0010 remains the target). → ticket 09
