...
- [ ] `schema_migrations` = 13 rows; seed present (18 sports, seed venues, demo accounts).
- [ ] User app loads; Firebase sign-in works (same Firebase tenant as local).
- [ ] Browse venues → venue detail renders photos through `/uploads` rewrite (no broken images).
...
- [ ] Admin photos: upload via venue form → image 200 on both apps' origins.
- [ ] **Volume persistence**: trigger a Railway redeploy → previously uploaded image still 200.
